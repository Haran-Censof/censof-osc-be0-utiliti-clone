...
class ApplicationSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        DB::table('osc_kod_aktvtagensi')->where('aka_kodaktiviti', 'A0087')->delete(); // Clean up specific test data
        ...
        // Insert Agency Activity Codes (Requested)
        $this->command->info('Seeding Agency Activity Codes (osc_kod_aktvtagensi)...');
        $agencyActivities = [
            ['aka_idpbt' => 'PRK_MDTM', 'aka_kodaktiviti' => 'A0087', 'aka_kodagensi' => 'JPBM', 'aka_statf' => 'Y'],
            ['aka_idpbt' => 'PRK_MDTM', 'aka_kodaktiviti' => 'A0087', 'aka_kodagensi' => 'JK', 'aka_statf' => 'Y'],
        ];

        foreach ($agencyActivities as $activity) {
            DB::table('osc_kod_aktvtagensi')->updateOrInsert(
                [
                    'aka_idpbt' => $activity['aka_idpbt'],
                    'aka_kodaktiviti' => $activity['aka_kodaktiviti'], 
                    'aka_kodagensi' => $activity['aka_kodagensi']
                ],
                array_merge($activity, [
                    'created_at' => Carbon::now(),
                    'updated_at' => Carbon::now(),
                    // 'aka_idate', 'aka_udate', 'aka_iuser', 'aka_uuser' are nullable in SQL provided
                ])
            );
        }
        ...
        for ($i = 0; $i < $totalApps; $i++) {
            ...
            $appData = [
                ...
                "mhn_namaperniagaan" => $app['business_name'],
            ...
            $trnRisiko = isset($niagaRecord->nia_risiko) ? $niagaRecord->nia_risiko : 'R';
