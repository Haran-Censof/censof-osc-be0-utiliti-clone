...
return new class extends Migration
{
    ...
    {
        Schema::create('osc_ind_lnomini', function (Blueprint $table) {
            $table->id('id')->comment('Primary Key');
            $table->string('nom_idpbt', 10)->nullable()->comment('ID PBT');
            $table->integer('nom_akaun')->nullable()->comment('NO AKAUN LESEN');
            ...
            $table->timestamps();
            $table->index(['nom_idpbt', 'nom_akaun']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('osc_ind_lnomini');
    }
};
